...
2.  Update OffscreenBrowserCompositorOutputSurface to use a shared texture for its FBO
	...
	it will use a shared texture as the color attachment for its FBO.
   ...
   3. in content/browser/offscreen_browser_compositor_output_surface.h 
      ...
      2. add the following declaration to OffscreenBrowserCompositorOutputSurface:
		...
		iii) add the following members to OffscreenBrowserCompositorOutputSurface
		
				bool shared_texture_enabled_ = false;
				uint64_t shared_handle_ = 0ull;
				uint32_t shared_texture_ = 0;

	d) in content/browser/offscreen_browser_compositor_output_surface.cc 
	
		i)  add the shared_texture_enabled to the ctor:
		...
		ii) add the implementation of GetSharedTexture:
		...
		iii) modify EnsureBackBuffer to create a shared texture for the FBO if it was enabled
		...
		iv) modify DiscardBackBuffer to tear-down the optional shared texture
		...
		v) modify BindFrameBuffer to optionally lock our shared texture
		...
		vi) modify swapbuffers to unlock our shared texture and issue a glFlush
